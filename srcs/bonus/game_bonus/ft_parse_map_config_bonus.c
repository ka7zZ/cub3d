...
{
	if (trim[0] == 'F' && ft_is_space(trim[1]) && !game->has_floor_color)
	{
		if (!ft_parse_rgb(trim + 2, &game->floor_color))
			return (0);
		return (game->has_floor_color = 1, 1);
	}
	if (trim[0] == 'C' && ft_is_space(trim[1]) && !game->has_ceiling_color)
	{
